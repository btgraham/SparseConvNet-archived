//don't delete the returned threads when done with them; ownership is with cnn.batchPool
...
void BatchProducer::preprocessBatch(int c, int cc, RNG& rng) {
  ...
  cnn.batchPool[cc].interfaces[0].featuresPresent.hVector()=range(dataset.nFeatures);
  ...
  for (int i=0;i<=cnn.layers.size();++i) {
    ...
    cnn.batchPool[cc].interfaces[i].rules.copyToGPUAsync(cnn.batchMemStreams[cc]);
  }
}
