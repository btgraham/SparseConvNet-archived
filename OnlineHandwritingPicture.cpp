...
std::vector<std::vector<float> > signatureWindows (arma::mat &path, int logSigDepth, int windowSize) {
  ...
  for (int i=0;i<path.size();++i) {
    ...
    logSignature(&repPath[3*first],last-first+1,3,logSigDepth,&sW[i][0]);
  }
  return sW;
}
...
void OnlineHandwritingPicture::codifyInputData(SparseGrid &grid, std::vector<float> &features, int &nSpatialSites, int spatialSize) {
  ...
  case UndirectedOctogram:
      ...
      for (int i=0; i<ops.size(); i++) {
        ...
        for (int j=0;j<csp.n_rows-1;++j) {
          ...
          if (a0>=0 and a1>=0 and a0<spatialSize and a1<spatialSize) {
            ...
            dy/=m;
            for (int k=0;k<4;k++) {
              float alpha=1-acosf(fabs(cosf(k*piBy4)*dx+sinf(k*piBy4)*dy))/piBy4;
              if (alpha>0)
                features[mpnnIF+k]+=alpha;
            }
          }
    ...
    break;
  case LogSignature1: logSigDepth=1; //todo: test
  case LogSignature2: logSigDepth=2;
  case LogSignature3: logSigDepth=3;
  case LogSignature4: logSigDepth=4;
    {
      int windowSize=4;
      ...
      for (int i=0; i<ops.size(); i++) {
        ...
        std::vector<std::vector<float> > sW=signatureWindows(CSP,logSigDepth,windowSize);
        for (int j=0;j<csp.n_rows;++j) {
          int J=j/multiplier;
          int
            a0=csp(j,0)+spatialSize/2.0,
