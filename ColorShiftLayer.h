class ColorShiftLayer : public SpatiallySparseLayer {
public:
  float sigma1;
  float sigma2;
  float sigma3;
  float sigma4;
  RNG rng;
  ColorShiftLayer(float sigma1=0, float sigma2=0, float sigma3=0, float sigma4=0) : sigma1(sigma1),  sigma2(sigma2), sigma3(sigma3),  sigma4(sigma4) {
    cout << "Color shift sigma = " << sigma1 << " " << sigma2 << " " << sigma3 <<" " << sigma4<<endl;
  };
  void preprocess
            ...
            for (int j=0;j<input.nFeatures;j++) {
              ...
              if (output.grids[i][k]!=bg) {
                output.features.hVector()[output.grids[i][k]*output.nFeatures+j]+=
                  delta1[j]+
                  delta2[j]*cos(output.features.hVector()[output.grids[i][k]*output.nFeatures+j]*3.1415926535/2)+
                  delta3[j]*(x*1.0f/input.spatialSize-0.5f)+
                  delta4[j]*(y*1.0f/input.spatialSize-0.5f);
              }
            }
          }
        }
