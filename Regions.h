...
public:
  ...
  //given a point in the output layer, location of "top-left" and "bottom-right"
  //corner of the corresponding pooling region in the input layer (left<=x<right)
...
public:
  int poolSize;
  int poolStride;
  RegularSquareRegions(int nIn, int nOut, int dimension, int poolSize, int poolStride);
...
template <typename ticks> class FractionalPoolingRegions : public RectangularRegions {
  std::vector<ticks> pb;
public:
  FractionalPoolingRegions(int nIn, int nOut, int dimension, int poolSize, RNG& rng);
...
public:
  ...
  int s;           // 1d pooling region size (same as poolSize in RegularTriangularRegions)
  ...
  RegularTriangularRegions(int nIn, int nOut, int dimension, int poolSize, int poolStride);
  //up to 4d
  //given a point in the output layer, location of "top-left" corner of the corresponding pooling region in the input layer
  int inputL(int j);
  //given an input-layer point, what is the range of points in the output layer to which it can be pooled ((left<=x<right))
  int outputL(int i);
  int outputR(int i);
};
...
void gridRules
(SparseGrid& inputGrid, //Keys 0,1,...,powf(regions.nIn,3)-1 represent grid points (plus paddding to form a square/cube)
 SparseGrid& outputGrid, //Keys 0,1,...,powf(regions.nOut,3)-1 represent grid points (plus paddding to form a square/cube)
