...
int readPotFile(std::vector<Picture*> &characters, const char* filename, int renderSize, OnlineHandwritingEncoding enc) {
  ...
  while (file.read((char*)&pCH,sizeof(potCharacterHeader))) {
    ...
    OnlineHandwritingPicture* character = new OnlineHandwritingPicture(renderSize, enc, label,0.0001);
...
SpatiallySparseDataset CasiaOLHWDB11TrainSet(int renderSize, OnlineHandwritingEncoding enc) {
...
SpatiallySparseDataset CasiaOLHWDB101112TrainSet(int renderSize, OnlineHandwritingEncoding enc) {
  SpatiallySparseDataset dataset;
...
SpatiallySparseDataset CasiaOLHWDB11TestSet(int renderSize, OnlineHandwritingEncoding enc) {
  SpatiallySparseDataset dataset;
