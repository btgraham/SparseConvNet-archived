...
trainingData=file("trainingData.txt__","w")
for x in csv.reader(file("2012_clsloc_classnames.txt"),delimiter=" "):
    ...
    for f in files:
        ...
        if isfile(f2):
            ...
            # ysize=input.shape[0]
            # xsize=input.shape[1]
            # print >>trainingData, x[0], f1, xsize, ysize, 0, xsize-1, 0, ysize-1
trainingData.close()
...
for x,c in zip(csv.reader(file("ILSVRC2012_devkit_t12/data/ILSVRC2012_validation_ground_truth.txt")),range(1,50001)):
    if not c in blacklist:
        ...
        ysize=input.shape[0]
        xsize=input.shape[1]
        print >>validationData, x[0], f1, 0, xsize-1, xsize-1, 0, ysize-1, ysize-1
...
for c in range(1,100001):
    ...
    ysize=input.shape[0]
    xsize=input.shape[1]
    print >>testData, c, f1, 0, xsize-1, xsize-1, 0, ysize-1, ysize-1
testData.close()
