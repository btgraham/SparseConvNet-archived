...
Picture* OpenCVPicture::distort(RNG& rng, batchType type) {
  ...
  float c00=1, c01=0;
  float c10=0, c11=1;
  c00=c11=300/scale*rng.uniform(0.8,1.2);  //scale diagonal to length 300 pixels
...
public:
  FractionalSparseConvNet(int nInputFeatures, int nClasses, int cudaDevice) : SparseConvNet(2,nInputFeatures, nClasses, cudaDevice) {
    ...
    for (int i=0;i<l;i++) {
      addLeNetLayerPOFMP(f(i),2,1,2,fmpShrink,VLEAKYRELU,0.5*std::max(i-4,0)/(l-2));
    }
    addLeNetLayerPOFMP(f(l),2,1,2,1.5,VLEAKYRELU,0.5*(l-4)/(l-2));
    addLeNetLayerMP(f(l+1),2,1,1,1,VLEAKYRELU,0.5*(l-3)/(l-2));
