...
DeepC3C3Valid::DeepC3C3Valid(int dimension, int l, int k, ActivationFunction fn,
                             int nInputFeatures, int nClasses, float p,
                             int cudaDevice, int nTop)
    : SparseConvNet(dimension, nInputFeatures, nClasses, cudaDevice, nTop) {
  for (int i = 0; i < l; i++) {
    addLeNetLayerMP((i + 1) * k, 3, 1, 1, 1, fn, p * i * 1.0f / l, -1);
    addLeNetLayerMP((i + 1) * k, 3, 1, 3, 2, fn, p * i * 1.0f / l, -1);
  }
  addLeNetLayerMP((l + 1) * k, 3, 1, 1, 1, fn, p,
                  +1); // Final "full" convolution just-in-case
  addLeNetLayerMP((l + 1) * k, 1, 1, 1, 1, fn, p);
  addSoftmaxLayer();
}
