...
int epoch=0;
int cudaDevice=2; // PCI bus ID: -1 for default GPU
int batchSize=20; // Increase/decrease according to GPU memory
#define VALIDATION
...
Picture* OpenCVPicture::distort(RNG& rng, batchType type) {
  ...
  float c00=1, c01=0;
  float c10=0, c11=1;
  c00=c11=200/scale*rng.uniform(0.8,1.2);  //scale diagonal to length 300 pixels
...
public:
  FractionalSparseConvNet(int nInputFeatures, int nClasses, int cudaDevice) : SparseConvNet(2,nInputFeatures, nClasses, cudaDevice) {
    ...
    for (int i=0;i<l;i++) {
      addLeNetLayerPOFMP(f(i),3,1,2,fmpShrink,VLEAKYRELU,0*std::max(i-4,0)/(l-2));
    }
    addLeNetLayerPOFMP(f(l),3,1,2,1.5,VLEAKYRELU,0*(l-4)/(l-2));
    addLeNetLayerMP(f(l+1),3,1,1,1,VLEAKYRELU,0*(l-3)/(l-2));
    addLeNetLayerMP(f(l+2),1,1,1,1,VLEAKYRELU,0.5);
    addSoftmaxLayer();
  }
};
...
int main() {
  std::string baseName="weights/kagglePlankton2";
